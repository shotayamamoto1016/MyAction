...
public class TakenokoSniper : MonoBehaviour, IResettable
{
    ...
    void ShootBullet()
    {
        ...
        // 現在の速度から発射角度を計算
        float currentAngle = Mathf.Atan2(vy, vx) * Mathf.Rad2Deg;

        // 角度を0〜180の範囲に補正
        if (currentAngle < 0) currentAngle += 360f;

        // 60度〜120度の範囲制限
        if (currentAngle > 60f && currentAngle < 90f)
        {
            // 85度などは60度に固定
            currentAngle = 60f;
        }
        else if (currentAngle >= 90f && currentAngle < 120f)
        {
            // 110度などは120度に固定
            currentAngle = 120f;
        }
        ...
        // 決定した角度に基づいて速度を再計算
        float speedMagnitude = new Vector2(vx, vy).magnitude; // 元の勢いを維持
        vx = Mathf.Cos(currentAngle * Mathf.Deg2Rad) * speedMagnitude;
        vy = Mathf.Sin(currentAngle * Mathf.Deg2Rad) * speedMagnitude;
    ...
    public void ResetObject()
    {
        ...
        {
            // emergeSpritesがインスペクターで1枚以上セットされているか確認
            if (emergeSprites != null && emergeSprites.Length > 0)
            {
